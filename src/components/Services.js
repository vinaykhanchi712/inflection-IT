// import React from "react";
// import img from "../images/web.svg";
// import img2 from "../images/app.svg";
// import img3 from "../images/hosting.svg";
// import img4 from "../images/consultation.svg";

// const Services = () => {
//   return (
//     <div id="services" className="bg-gray-100 py-12">
//       <section data-aos="zoom-in-down">
//         <div className="my-4 py-4">
//           <h2 className="my-2 text-center text-3xl text-blue-900 uppercase font-bold">
//             services
//           </h2>

//           <div className="flex justify-center">
//             <div className="w-24 border-b-4 border-blue-900"></div>
//           </div>
//           <h2 className="mt-4 mx-12 text-center text-xl lg:text-2xl font-semibold text-blue-900">
//             We are deeply committed to the growth and success of our clients.
//           </h2>
//         </div>

//         <div className="px-12" data-aos="fade-down" data-aos-delay="600">
//           <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
//             <div className="bg-white transition-all ease-in-out duration-400  overflow-hidden text-gray-700 hover:bg-gray-500 hover:text-white rounded-lg shadow-2xl p-3 group">
//               <div className="m-2 text-justify text-sm">
//                 <img
//                   alt="card img"
//                   className="rounded-t group-hover:scale-[1.15] transition duration-1000 ease-in-out"
//                   src={img}
//                 />
//                 <h2 className="font-semibold my-4 text-2xl text-center">
//                   Web Development
//                 </h2>
//                 <p className="text-md font-medium">
//                   We specialize in creating and optimizing high-quality, custom
//                   websites for businesses and organizations of all sizes.
//                   Building mobile-friendly and easy-to-use websites and
//                   applications for clients.
//                 </p>
//               </div>
//             </div>

//             <div className="bg-white transition-all ease-in-out duration-400  overflow-hidden text-gray-700 hover:bg-gray-500 hover:text-white rounded-lg shadow-2xl p-3 group">
//               <div className="m-2 text-justify text-sm">
//                 <img
//                   alt="card img"
//                   className="rounded-t group-hover:scale-[1.15] transition duration-1000 ease-in-out"
//                   src={img2}
//                 />
//                 <h2 className="font-semibold my-4 text-2xl text-center">
//                   Mobile App Development
//                 </h2>
//                 <p className="text-md font-medium">
//                   We develop high-quality, custom cross-platform mobile
//                   applications that are robust and optimized for performance,
//                   scalability, and accessibility.
//                 </p>
//               </div>
//             </div>

//             <div className="bg-white transition-all ease-in-out duration-400  overflow-hidden text-gray-700 hover:bg-gray-500 hover:text-white rounded-lg shadow-2xl p-3 group">
//               <div className="m-2 text-justify text-sm">
//                 <img
//                   alt="card img"
//                   className="rounded-t group-hover:scale-[1.15] transition duration-1000 ease-in-out"
//                   src={img3}
//                 />
//                 <h2 className="font-semibold my-4 text-2xl text-center ">
//                   Domain and Hosting Services
//                 </h2>
//                 <p className="text-md font-medium">
//                   We provide domain registration and web hosting services to
//                   individuals and organizations to enable them gain visibility
//                   in the digital space.
//                 </p>
//               </div>
//             </div>

//             <div className="bg-white transition-all ease-in-out duration-400  overflow-hidden text-gray-700 hover:bg-gray-500 hover:text-white rounded-lg shadow-2xl p-3 group">
//               <div className="m-2 text-justify text-sm">
//                 <img
//                   alt="card img"
//                   className="rounded-t group-hover:scale-[1.15] transition duration-1000 ease-in-out"
//                   src={img3}
//                 />
//                 <h2 className="font-semibold my-4 text-2xl text-center ">
//                   Digital Marketing
//                 </h2>
//                 <p className="text-md font-medium">
//                   We provide result-driven digital marketing solutions to help brands grow online.
//                   Our strategies focus on visibility, engagement, and measurable business impact.
//                 </p>
//               </div>
//             </div>

//             <div className="bg-white transition-all ease-in-out duration-400  overflow-hidden text-gray-700 hover:bg-gray-500 hover:text-white rounded-lg shadow-2xl p-3 group">
//               <div className="m-2 text-justify text-sm">
//                 <img
//                   alt="card img"
//                   className="rounded-t group-hover:scale-[1.15] transition duration-1000 ease-in-out"
//                   src={img3}
//                 />
//                 <h2 className="font-semibold my-4 text-2xl text-center ">
//                   Cloud Solutions
//                 </h2>
//                 <p className="text-md font-medium">
//                   We deliver secure and scalable cloud solutions tailored to your business needs.
//                   From cloud migration to optimization, we help you build for the future.
//                 </p>
//               </div>
//             </div>

//             <div className="bg-white transition-all ease-in-out duration-400  overflow-hidden text-gray-700 hover:bg-gray-500 hover:text-white rounded-lg shadow-2xl p-3 group">
//               <div className="m-2 text-justify text-sm">
//                 <img
//                   alt="card img"
//                   className="rounded-t group-hover:scale-[1.15] transition duration-1000 ease-in-out"
//                   src={img3}
//                 />
//                 <h2 className="font-semibold my-4 text-2xl text-center ">
//                   Logo and Branding Design
//                 </h2>
//                 <p className="text-md font-medium">
//                   We provide domain registration and web hosting services for individuals and organizations.
//                   Helping them establish a strong and reliable presence in the digital space.
//                 </p>
//               </div>
//             </div>

//             <div className="bg-white transition-all ease-in-out duration-400  overflow-hidden text-gray-700 hover:bg-gray-500 hover:text-white rounded-lg shadow-2xl p-3 group">
//               <div className="m-2 text-justify text-sm">
//                 <img
//                   alt="card img"
//                   className="rounded-t group-hover:scale-[1.15] transition duration-1000 ease-in-out"
//                   src={img4}
//                 />
//                 <h2 className="font-semibold my-4 text-2xl text-center ">
//                   General IT Consultations
//                 </h2>
//                 <p className="text-md font-medium">
//                   Our IT consultations service provides expert guidance and
//                   support for businesses and individuals looking to optimize
//                   their technology solutions.
//                 </p>
//               </div>
//             </div>
//           </div>
//         </div>
//       </section>

//       <section>
//         <div className="m-auto max-w-6xl p-2 md:p-12 h-5/6">
//           <div
//             className="flex flex-col-reverse lg:flex-row py-8 justify-between lg:text-left"
//             data-aos="zoom-out"
//           >
//             <div className="lg:w-1/2 flex flex-col lg:mx-4 justify-center">
//               <div className="text-blue-900 mb-4">
//                 <svg
//                   xmlns="http://www.w3.org/2000/svg"
//                   width="72"
//                   height="72"
//                   viewBox="0 0 24 24"
//                   className="fill-current"
//                 >
//                   <path d="M2 12h2a7.986 7.986 0 0 1 2.337-5.663 7.91 7.91 0 0 1 2.542-1.71 8.12 8.12 0 0 1 6.13-.041A2.488 2.488 0 0 0 17.5 7C18.886 7 20 5.886 20 4.5S18.886 2 17.5 2c-.689 0-1.312.276-1.763.725-2.431-.973-5.223-.958-7.635.059a9.928 9.928 0 0 0-3.18 2.139 9.92 9.92 0 0 0-2.14 3.179A10.005 10.005 0 0 0 2 12zm17.373 3.122c-.401.952-.977 1.808-1.71 2.541s-1.589 1.309-2.542 1.71a8.12 8.12 0 0 1-6.13.041A2.488 2.488 0 0 0 6.5 17C5.114 17 4 18.114 4 19.5S5.114 22 6.5 22c.689 0 1.312-.276 1.763-.725A9.965 9.965 0 0 0 12 22a9.983 9.983 0 0 0 9.217-6.102A9.992 9.992 0 0 0 22 12h-2a7.993 7.993 0 0 1-.627 3.122z"></path>
//                   <path d="M12 7.462c-2.502 0-4.538 2.036-4.538 4.538S9.498 16.538 12 16.538s4.538-2.036 4.538-4.538S14.502 7.462 12 7.462zm0 7.076c-1.399 0-2.538-1.139-2.538-2.538S10.601 9.462 12 9.462s2.538 1.139 2.538 2.538-1.139 2.538-2.538 2.538z"></path>
//                 </svg>
//               </div>
//               <h3
//                 className="text-3xl  text-blue-900 
//                             font-bold"
//               >
//                 We <span className="font-black">Build</span>
//               </h3>
//               <div>
//                 <p className="my-3 text-xl text-gray-600 font-semibold">
//                   With over 10 years experience in software analysis and design
//                   and a deep understanding of the latest IT trends and
//                   solutions, we provide customized recommendations and
//                   strategies to help you improve your operations, reduce costs,
//                   and increase efficiency.
//                 </p>
//               </div>
//             </div>
//             <div className="lg:w-1/2 flex flex-col lg:mx-4 justify-center">
//               <div className="text-blue-900 mb-4">
//                 <svg
//                   xmlns="http://www.w3.org/2000/svg"
//                   width="72"
//                   height="72"
//                   viewBox="0 0 24 24"
//                   className="fill-current"
//                 >
//                   <path d="m7.375 16.781 1.25-1.562L4.601 12l4.024-3.219-1.25-1.562-5 4a1 1 0 0 0 0 1.562l5 4zm9.25-9.562-1.25 1.562L19.399 12l-4.024 3.219 1.25 1.562 5-4a1 1 0 0 0 0-1.562l-5-4zm-1.649-4.003-4 18-1.953-.434 4-18z"></path>
//                 </svg>
//               </div>
//               <h3 className="text-3xl  text-blue-900 font-bold">
//                 We <span className="font-black">Collaborate</span>
//               </h3>
//               <div>
//                 <p className="my-3 text-xl text-gray-600 font-semibold">
//                   We can collaborate with your existing tech team to scale
//                   existing software applications or design customized software
//                   applications that suits your everyday need and simplifies
//                   various processes.
//                 </p>
//               </div>
//             </div>
//           </div>
//         </div>
//       </section>
//     </div>
//   );
// };

// export default Services;
import React, { useRef, useEffect, useState } from "react";
import img from "../images/web.svg";
import img2 from "../images/app.svg";
import img3 from "../images/hosting.svg";
import img4 from "../images/digital-marketing.svg"
import img5 from "../images/cloud-solutions.svg"
import img6 from "../images/branding.svg"
import img7 from "../images/consultation.svg";

const ServiceCard = ({ img, title, children }) => {
  return (
    <article className="service-card bg-white rounded-lg shadow-2xl p-4 transition-transform duration-300 flex-shrink-0">
      <div className="service-img mb-3 rounded overflow-hidden">
        <img src={img} alt={title} className="w-full h-full object-contain" />
      </div>

      <h4 className="text-lg text-center font-semibold mb-2 text-blue-900">{title}</h4>
      <p className="text-sm text-gray-600 line-clamp-4">{children}</p>
    </article>
  );
};

const ServicesCarousel = ({ children, autoDelay = 2200 }) => {
  const outerRef = useRef(null);
  const innerRef = useRef(null);
  const [step, setStep] = useState(0);
  const [index, setIndex] = useState(0);
  const [isPaused, setIsPaused] = useState(false);
  const pauseRef = useRef(false);

  // measure step size (distance between starts of consecutive cards)
  useEffect(() => {
    const inner = innerRef.current;
    if (!inner) return;
    const childrenNodes = Array.from(inner.children);
    if (childrenNodes.length < 2) {
      // fallback to child's width
      const w = childrenNodes[0]?.getBoundingClientRect().width || 300;
      setStep(w);
      return;
    }
    const rect0 = childrenNodes[0].getBoundingClientRect();
    const rect1 = childrenNodes[1].getBoundingClientRect();
    const measured = Math.abs(rect1.left - rect0.left);
    setStep(Math.round(measured));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [children]);

  // auto-advance logic: scroll to next index every autoDelay ms
  useEffect(() => {
    if (!step) return;
    let timer = null;
    function advance() {
      if (pauseRef.current) return;
      const inner = innerRef.current;
      if (!inner) return;
      const maxIndex = Math.max(0, inner.children.length - 1);
      const next = index >= maxIndex ? 0 : index + 1;
      setIndex(next);
      if (outerRef.current) {
        outerRef.current.scrollTo({
          left: next * step,
          behavior: "smooth",
        });
      }
    }

    timer = setInterval(advance, autoDelay);
    return () => clearInterval(timer);
  }, [step, index, autoDelay]);

  // when user manually clicks prev/next, update index and scroll
  const goTo = (next) => {
    const inner = innerRef.current;
    if (!inner) return;
    const maxIndex = Math.max(0, inner.children.length - 1);
    let n = next;
    if (n < 0) n = maxIndex;
    if (n > maxIndex) n = 0;
    setIndex(n);
    if (outerRef.current) {
      outerRef.current.scrollTo({
        left: n * step,
        behavior: "smooth",
      });
    }
    // pause briefly after manual interaction
    pauseRef.current = true;
    setIsPaused(true);
    clearTimeout(pauseRef.timeout);
    pauseRef.timeout = setTimeout(() => {
      pauseRef.current = false;
      setIsPaused(false);
    }, 3500);
  };

  // pause on hover/focus
  const handleMouseEnter = () => {
    pauseRef.current = true;
    setIsPaused(true);
  };
  const handleMouseLeave = () => {
    pauseRef.current = false;
    setIsPaused(false);
  };

  return (
    <div className="relative">
      <button
        aria-label="Scroll left"
        className="carousel-button left-2 hidden md:flex"
        onClick={() => goTo(index - 1)}
      >
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M12 15L7 10l5-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
      </button>

      <div
        ref={outerRef}
        className="overflow-x-auto no-scrollbar"
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
        onFocus={handleMouseEnter}
        onBlur={handleMouseLeave}
        role="region"
        aria-label="Services carousel"
        style={{ scrollSnapType: "x mandatory", WebkitOverflowScrolling: "touch" }}
      >
        <div ref={innerRef} className="flex gap-5 items-stretch px-4 py-6" style={{ scrollSnapType: "x mandatory" }}>
          {React.Children.map(children, (child) => (
            <div style={{ scrollSnapAlign: "start" }} className="flex-shrink-0">
              {child}
            </div>
          ))}
        </div>
      </div>

      <button
        aria-label="Scroll right"
        className="carousel-button right-2 hidden md:flex"
        onClick={() => goTo(index + 1)}
      >
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M8 5l5 5-5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
      </button>

      {/* small indicator / pause state (optional) */}
      <div className="sr-only" aria-hidden={!isPaused}>
        {isPaused ? "Auto-scroll paused" : "Auto-scroll active"}
      </div>
    </div>
  );
};

const Services = () => {
  const items = [
    {
      img,
      title: "Web Development",
      desc:
        "We specialize in creating and optimizing high-quality, custom websites for businesses and organizations of all sizes. Building mobile-friendly and easy-to-use websites and applications for clients.",
    },
    {
      img: img2,
      title: "Mobile App Development",
      desc:
        "We develop high-quality, custom cross-platform mobile applications that are robust and optimized for performance, scalability, and accessibility.",
    },
    {
      img: img3,
      title: "Domain & Hosting",
      desc:
        "We provide domain registration and web hosting services to individuals and organizations to enable them gain visibility in the digital space.",
    },
    {
      img: img4,
      title: "Digital Marketing",
      desc:
        "We provide result-driven digital marketing solutions to help brands grow online. Our strategies focus on visibility, engagement, and measurable business impact.",
    },
    {
      img: img5,
      title: "Cloud Solutions",
      desc:
        "We deliver secure and scalable cloud solutions tailored to your business needs. From cloud migration to optimization, we help you build for the future.",
    },
    {
      img: img6,
      title: "Branding & Logo",
      desc:
        "Branding and visual identity services that help your business stand out and communicate value clearly.",
    },
    {
      img: img7,
      title: "IT Consultations",
      desc:
        "Our IT consultations service provides expert guidance and support for businesses and individuals looking to optimize their technology solutions.",
    },
  ];

  return (
    <div id="services" className="bg-gray-100 py-12">
      <section data-aos="zoom-in-down">
        <div className="my-4 py-4">
          <h2 className="my-2 text-center text-3xl text-blue-900 uppercase font-bold">services</h2>

          <div className="flex justify-center">
            <div className="w-24 border-b-4 border-blue-900" />
          </div>
          <h2 className="mt-4 mx-12 text-center text-xl lg:text-2xl font-semibold text-blue-900">
            We are deeply committed to the growth and success of our clients.
          </h2>
        </div>

        <div className="px-2 md:px-12" data-aos="fade-down" data-aos-delay="600">
          <ServicesCarousel autoDelay={2600}>
            {items.map((it, idx) => (
              <ServiceCard key={idx} img={it.img} title={it.title}>
                {it.desc}
              </ServiceCard>
            ))}
          </ServicesCarousel>
        </div>
      </section>

      {/* preserved lower informational section */}
      <section>
        <div className="m-auto max-w-6xl p-2 md:p-12 h-5/6">
          <div className="flex flex-col-reverse lg:flex-row py-8 justify-between lg:text-left" data-aos="zoom-out">
            <div className="lg:w-1/2 flex flex-col lg:mx-4 justify-center">
              <div className="text-blue-900 mb-4">
                {/* svg omitted for brevity */}
                <svg xmlns="http://www.w3.org/2000/svg" width="72" height="72" viewBox="0 0 24 24" className="fill-current">
                  <path d="M2 12h2a7.986 7.986 0 0 1 2.337-5.663 7.91 7.91 0 0 1 2.542-1.71 8.12 8.12 0 0 1 6.13-.041A2.488 2.488 0 0 0 17.5 7C18.886 7 20 5.886 20 4.5S18.886 2 17.5 2c-.689 0-1.312.276-1.763.725-2.431-.973-5.223-.958-7.635.059a9.928 9.928 0 0 0-3.18 2.139 9.92 9.92 0 0 0-2.14 3.179A10.005 10.005 0 0 0 2 12zm17.373 3.122c-.401.952-.977 1.808-1.71 2.541s-1.589 1.309-2.542 1.71a8.12 8.12 0 0 1-6.13.041A2.488 2.488 0 0 0 6.5 17C5.114 17 4 18.114 4 19.5S5.114 22 6.5 22c.689 0 1.312-.276 1.763-.725A9.965 9.965 0 0 0 12 22a9.983 9.983 0 0 0 9.217-6.102A9.992 9.992 0 0 0 22 12h-2a7.993 7.993 0 0 1-.627 3.122z" />
                  <path d="M12 7.462c-2.502 0-4.538 2.036-4.538 4.538S9.498 16.538 12 16.538s4.538-2.036 4.538-4.538S14.502 7.462 12 7.462zm0 7.076c-1.399 0-2.538-1.139-2.538-2.538S10.601 9.462 12 9.462s2.538 1.139 2.538 2.538-1.139 2.538-2.538 2.538z" />
                </svg>
              </div>
              <h3 className="text-3xl text-blue-900 font-bold">
                We <span className="font-black">Build</span>
              </h3>
              <div>
                <p className="my-3 text-xl text-gray-600 font-semibold">
                  With experience in software analysis and design and a deep understanding of the latest IT trends and solutions, we provide customized recommendations and strategies to help you improve your operations, reduce costs, and increase efficiency.
                </p>
              </div>
            </div>

            <div className="lg:w-1/2 flex flex-col lg:mx-4 justify-center">
              <div className="text-blue-900 mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" width="72" height="72" viewBox="0 0 24 24" className="fill-current">
                  <path d="m7.375 16.781 1.25-1.562L4.601 12l4.024-3.219-1.25-1.562-5 4a1 1 0 0 0 0 1.562l5 4zm9.25-9.562-1.25 1.562L19.399 12l-4.024 3.219 1.25 1.562 5-4a1 1 0 0 0 0-1.562l-5-4zm-1.649-4.003-4 18-1.953-.434 4-18z" />
                </svg>
              </div>
              <h3 className="text-3xl text-blue-900 font-bold">
                We <span className="font-black">Collaborate</span>
              </h3>
              <div>
                <p className="my-3 text-xl text-gray-600 font-semibold">
                  We can collaborate with your existing tech team to scale existing software applications or design customized software applications that suits your everyday need and simplifies various processes.
                </p>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
};

export default Services;